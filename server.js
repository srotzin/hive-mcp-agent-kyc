...
const SUBSCRIPTION_TIERS = {
  starter:    { price_usd: 20.0, calls_per_day: 100, label: 'Starter' },
  pro:        { price_usd: 100.0, calls_per_day: 1000, label: 'Pro' },
...
async function emitSpectralReceipt({ event_type, did, amount_usd, tool_name, tx_hash, metadata }) {
  ...
  try {
    ...
    console.warn('[agent-kyc] receipt emit failed (non-fatal):', _.message || _);
  }
}
...
app.post('/v1/subscription', async (req, res) => {
  ...
  if (tier !== 'enterprise' && !tx_hash) {
    return res.status(402).json({
      ...
      x402: {
        type: 'x402', version: '1', kind: 'subscription_agent-kyc',
  ...
  await emitSpectralReceipt({
    ...
    tx_hash: tx_hash || null,
    metadata: { tier, service: 'Hive Agent KYC', expires_ms: record.expires_ms },
  });
  ...
  return res.json({
    ...
    partner_attribution: 'Agent KYC broker layer complements Chainalysis, TRM, Elliptic — Hive routes and attests, partners verify',
    brand: '#C08D23',
